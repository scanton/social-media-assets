...
export const HELP: Record<string, HelpEntry> = {
  ...
  "scene.background": {
    ...
    body:
      "Skip the generated scene and drop in a photograph you already have. The card gets composited into it, and so is whoever is presenting it. Choosing one locks only the three things the picture genuinely decides — the setting, the lighting and the film look. Everything else stays yours: who is in frame and what they look like, how the card is held, how close the camera gets, and which way the card is turned to it — flat lay included, since a top-down photograph of a surface is exactly the background a flat lay wants. On a digital card the clip is also asked to bring the photograph to life rather than hold it still, so the world in it moves while the place stays the same. What the photograph does not decide either is the shape: the orientations you tick below still set that, so a photograph of a different ratio is cropped to fit, and a wide shot picked for a 9:16 render loses its sides. Match the shape you are rendering and the framing you chose is the framing you get. These are the sizes each orientation is rendered at, largest quality to smallest — supply the shape, and at least the larger size if you have it.",
  ...
  "bench.settings": {
    ...
    body:
      "These are read from the model's own published schema, not written by us, which is why they change completely when you switch models and why the wording is sometimes the vendor's rather than ours. Anything left alone uses that model's own default, with one or two exceptions where we open on something more sensible — the GPT Image models start at medium quality rather than their own default, because the difference is hard to see at social sizes and easy to see on the bill — and GPT Image 2.5 added two tiers above high.",
  },
  "bench.compiled": {
